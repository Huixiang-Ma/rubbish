  ...
  <div class="container book-page">
    ...
    <template v-else-if="!trip">
      <div class="empty card big"><div class="icon">🧳</div><p>没有找到这份行程书，可能已被删除</p>
        <div class="act">
          ...
          <router-link :to="{ name: 'plans' }" class="btn btn-soft">去新建规划</router-link>
        </div>
      </div>
    </template>
    ...
    <template v-else>
      ...
      <div class="book-hero">
        <div class="bh-main">
          ...
          <div class="bh-info">
            <div class="bh-tags">
              ...
              <span v-if="trip.source === 'job'" class="tag-teal" title="由 AI 任务转正而来（下轮开放）">🤖 AI 任务转正</span>
            </div>
            ...
            <div class="bh-meta">
              ...
              <span v-if="updatedText" class="meta-mute">· 更新于 {{ updatedText }}</span>
            </div>
        ...
        <div class="bh-side">
          ...
          <div class="bh-actions">
            <router-link :to="{ name: 'trip-map', params: { id: trip.id } }" class="btn btn-ghost">🗺 地图 / 时间线</router-link>
            <router-link :to="{ name: 'trip-edit', params: { id: trip.id } }" class="btn btn-primary">✏️ 编辑</router-link>
            <button class="btn btn-ghost" @click="onDelete">🗑 删除</button>
          </div>
        </div>
      </div>
      ...
      <div class="day-book">
        <section v-for="d in trip.dayPlans" :key="d.day" class="db-day">
          <header class="db-head">
            ...
            <div class="db-titles">
              <div class="db-title">第 {{ d.day }} 天<em v-if="dayAbsDate(d.day)">{{ dayAbsDate(d.day) }}</em></div>
      ...
      <div class="book-foot">
        ...
        <router-link :to="{ name: 'plans' }" class="link-more">✦ 再规划一条行程 →</router-link>
      </div>
    </template>
  </div>
</template>
...
<script setup>
// P2-只读页：从原 TripBookView 剥离 editing 态
//   - 路由 /trip/:id
//   - 数据来源：Pinia trips store（useTripsStore）
//   - 行为：仅展示 + 跳转 编辑/地图/商城/删除
//   - 不再接收 ?mode=edit query（编辑走独立路由 /trip/:id/edit）
...
const updatedText = computed(() => {
  const u = trip.value?.updated_at || ''
  return u ? u.replace('T', ' ').slice(0, 16) : ''
})

function load() {
  store.load()
  trip.value = store.byId(String(route.params.id || ''))
  loading.value = false
}
...
function daySummary(d) {
  ...
  const total = (d.blocks || []).filter(b => b.period !== 'night').length
  return names.length ? names.join(' → ') + (total > 2 ? ' 等' : '') : '自由安排'
}
function dayAbsDate(day) {
...
.tag-teal { background: #F0FDFA; color: #0F766E; border: 1px solid #99F6E4; font-size: 11.5px; padding: 3px 10px; border-radius: 999px; font-weight: 600; }
...
.meta-mute { color: var(--ink-400); font-size: 12px; }
...
.db-titles { min-width: 0; }
.db-title { font-weight: 800; font-size: 16px; color: var(--ink-900); }
.db-title em { font-style: normal; font-size: 12px; color: var(--ink-400); font-weight: 600; margin-left: 8px; font-family: var(--mono); }
